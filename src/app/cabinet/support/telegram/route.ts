...
export async function POST(req: Request) {
    try {
        ...
        const anonKey = envStrict("NEXT_PUBLIC_SUPABASE_ANON_KEY");

        // JWT пользователя (или админа) — чтобы запросы шли под RLS
        const authHeader = req.headers.get("authorization");
        if (!authHeader?.toLowerCase().startsWith("bearer ")) {
            // не ломаем UX
            return NextResponse.json({ ok: true, skipped: true, reason: "missing bearer token" });
        }
        ...
        if (tErr || !t) {
            return NextResponse.json({ ok: false, error: tErr?.message ?? "thread not found" }, { status: 404 });
        }
