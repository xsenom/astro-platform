        ...
        try:
            ...
            if loc:
                ...
                if not (-90 <= lat <= 90) or not (-180 <= lon <= 180):
                    raise ValueError(f"Невалидные координаты: lat={lat}, lon={lon}")
                print(f"[DEBUG] Найдено: {variant} → {lat}, {lon}")
                return lat, lon
        except Exception as e:
            print(f"[DEBUG] Ошибка геокодирования '{variant}': {e}")
            continue

    # 🔄 "Грязный" перебор словосочетаний
    words = city_name.replace(",", " ").split()
    for size in range(len(words), 1, -1):
        for i in range(len(words) - size + 1):
            subphrase = " ".join(words[i:i+size])
            try:
